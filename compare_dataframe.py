...
df1 = spark.read.option("header", True).option("delimiter", ",").option("quote", '"').option("escape", '"').csv("/final_data3.csv")
df2 = spark.read.option("header", True).option("delimiter", ",").option("quote", '"').option("escape", '"').csv("/final_data1.csv")
...
primary_keys = ["id", "num"]  # Replace with your primary key columns
